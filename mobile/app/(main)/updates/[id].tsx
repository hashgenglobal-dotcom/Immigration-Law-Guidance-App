...
export default function UpdateDetailScreen() {
  ...
  const { id, askTopic } = useLocalSearchParams<{ id: string; askTopic?: string }>()
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await fetchUpdateDetail(
        updateId,
        typeof askTopic === 'string' && askTopic ? askTopic : null,
      )
  ...
  const openAsk = useCallback(() => {
    if (!item?.ask_prefill_message) return
    router.push({
      pathname: '/ask',
      params: { prefill: item.ask_prefill_message },
    })
  }, [item, router])
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          >
            ...
            <Text style={styles.secondaryBtnText}>Ask how this affects my situation</Text>
          </Pressable>

          <Text style={styles.askNote}>
            Ask searches statutes, regulations, and USCIS policy manual sources—not this press
            release. Your prefilled question is kept in memory only on this device.
          </Text>
        </ScrollView>
      )}
    </View>
