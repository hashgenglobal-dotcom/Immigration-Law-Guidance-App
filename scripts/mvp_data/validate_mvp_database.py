...
def _dataset_versions_cols(cur: Any) -> frozenset[str]:
    """Return the column names that actually exist in dataset_versions."""
    cur.execute(
        """
        SELECT column_name
...
def _col_or_null(col: str, existing: frozenset[str]) -> str:
    """Return 'col' if present in the table, else 'NULL AS col'."""
    return col if col in existing else f"NULL AS {col}"
...
def run_checks(cur: Any) -> tuple[list[str], list[str], dict[str, Any]]:
    ...
    #
    # Introspect the schema first so the SELECT never references a column
    # that does not exist (e.g. some installs have 'created_at', others
    # use 'started_at' / 'completed_at').  The three columns required for
    # all logic checks — id, version_name, status — are always selected
    # by name.  Optional metadata columns get NULL if absent.
    # ------------------------------------------------------------------
    existing = _dataset_versions_cols(cur)

    # Optional columns supported by the current known schema variants.
    optional = ("started_at", "completed_at", "activated_at", "created_by")
    select_extras = ",\n        ".join(_col_or_null(c, existing) for c in optional)

    # Order by 'id' — always present and monotonically increasing.
    cur.execute(
        f"""
        SELECT
            id,
...
def _print_report(details: dict[str, Any], failures: list[str], warnings: list[str]) -> None:
    ...
    if not all_ds:
        ...
        hdr = f"  {'version_name':<42}  {'status':<10}  {'started_at':<26}  {'activated_at'}"
        print(hdr)
        print("  " + "-" * (len(hdr) - 2))
        for d in all_ds:
            started = d.get("started_at") or "—"
            activated = d.get("activated_at") or "—"
            print(f"  {d['version_name']:<42}  {d['status']:<10}  {started:<26}  {activated}")
