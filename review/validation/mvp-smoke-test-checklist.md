...
| # | Check | Command / action | Pass criteria |
|---|--------|------------------|---------------|
| 5.1 | Mobile grep | Run the standard mobile secret scan from the review workflow | No matches in app source |
| 5.2 | Mobile storage | `grep -r 'AsyncStorage.setItem' mobile/src` | Only `AuthContext` / onboarding + guest count keys |
| 5.3 | Review docs grep | Run the standard review-doc safety scan from the review workflow | No real secrets (pattern in this checklist line is OK) |
