...
During Rishi's local review, the backend code compiled and the MVP source-scope helper checks passed, but full endpoint validation could not be completed on Rishi's local database.

Rishi's local Postgres database currently contains only the legacy sample dataset:

- `ecfr-title8-sample-2026-05-11`
- 5 chunks
- embeddings stored directly in `legal_chunks.embedding`
- no full eCFR / INA / USCIS PM dataset rows present locally

Because of that local data mismatch, `/api/retrieve` and `/api/chat` still returned the sample dataset during Rishi's endpoint test. This does not necessarily invalidate the backend routing code, but it means MVP data readiness must be re-tested after the full MVP database is recreated locally or restored from a reproducible handoff.

Required follow-up before final MVP sign-off:

1. Recreate or restore the full MVP database locally.
2. Confirm active datasets:
   - `ecfr-title8-full-*`
   - `ina-*`
   - `uscis-pm-*`
3. Confirm the sample dataset is not active.
4. Re-run `/api/retrieve` and `/api/chat` source-reporting tests.
5. Confirm `active_dataset` reports `mvp-multi-source: ...`.
