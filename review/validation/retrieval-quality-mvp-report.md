...
> During Rishi's local review, retrieval scoring unit tests passed, but full golden retrieval
> validation could not be reproduced because local PostgreSQL still contains only the sample eCFR
> dataset (`ecfr-title8-sample-2026-05-11`, 5 chunks). The 13/13 top-3 results in this report are
> teammate/full-data-environment results and must be re-run after full MVP DB rebuild/import.
> To reproduce locally: follow the MVP DB rebuild guide (`scripts/mvp_data/README.md`) to import
> the full corpus, then re-run `validate_mvp_golden_retrieval.py --output`.

---

## Summary

| Metric | Before tuning | After tuning (teammate/full-data environment) |
|--------|---------------|----------------------------------------------|
| Golden queries top-3 hit | 8 / 13 (62%) | **13 / 13 (100%)** — pending local reproduction |
| Golden queries top-1 hit | 6 / 13 (46%) | **11 / 13 (85%)** — pending local reproduction |
| Legacy 5-query exact citation in top-5 | NTA missing | All present (NTA § 239.1 at rank 3) — pending local reproduction |
...
# On sample-only local DB (exits 0 regardless of score):
...
| Criterion | Status |
|-----------|--------|
| Expected source in top 3 for most golden queries | Validated after import in teammate/full-data environment: **13/13** — pending local reproduction after full MVP DB rebuild |
| Broad EAD can still trigger guided intake | Unchanged (`guided_intake.py`) |
| No regression on strong queries (asylum EAD, forms, address) | Improved or stable (teammate env) |
| Backend unit tests pass | `test_retrieval_scoring.py` — passes on local sample DB |
| Full golden validation on Rishi's local DB | **Pending local DB rebuild** |
