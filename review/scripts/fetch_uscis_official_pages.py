...
# Committed fallback — used by --inspect when the data/ copy is not yet present locally.
INVENTORY_FALLBACK = Path("review/validation/common-immigration-source-inventory.json")
RAW_DIR = Path("data/uscis-official-pages/raw")
PREVIEW_FILE = Path("data/uscis-official-pages/preview/pages-preview.json")

USER_AGENT = "SourcePath-MVP-SourceFetcher/0.1 (educational legal source validation)"
HTTP_TIMEOUT = 60.0
RATE_LIMIT_SEC = 1.0


def _load_urls(limit: int | None) -> list[dict[str, Any]]:
    inv_path = INVENTORY if INVENTORY.is_file() else INVENTORY_FALLBACK
    if not inv_path.is_file():
        raise SystemExit(f"ERROR: inventory not found at {INVENTORY} or {INVENTORY_FALLBACK}")
...
def _extract_text(html: str) -> tuple[str, str]:
    try:
        ...
        raise SystemExit(
            "ERROR: beautifulsoup4 (bs4) is required for HTML parsing.\n"
            "       Run with: uv run --project backend python review/scripts/fetch_uscis_official_pages.py"
        )
...
async def _run(limit: int | None, write: bool) -> dict[str, Any]:
    ...
    try:
        ...
        raise SystemExit(
            "ERROR: httpx is required for fetching pages.\n"
            "       Run with: uv run --project backend python review/scripts/fetch_uscis_official_pages.py"
        )
