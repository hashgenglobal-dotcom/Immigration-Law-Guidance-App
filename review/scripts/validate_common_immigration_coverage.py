...
async def run_validation(output: bool) -> dict:
    ...
    for qid, query, cat_id in VALIDATION_QUERIES:
        results, _active_datasets, active_dataset = await svc.retrieve_hybrid(query, top_k=5)
    ...
    if INVENTORY.is_file():
        ...
        print(f"WARN: source inventory not found at {INVENTORY} — skipping category status table")
