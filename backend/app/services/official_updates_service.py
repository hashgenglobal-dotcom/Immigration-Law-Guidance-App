...
def build_ask_prefill_message(
    *,
    title: str,
    publisher: str,
    official_url: str,
    published_at: str,
    topic_label_for_button: str,
) -> str:
    pub_display = publisher.replace("_", " ").title()
    return (
        f"An official {pub_display} announcement published on {published_at[:10]} is titled: "
        f"\"{title}\". Based only on immigration statutes, regulations, and USCIS policy "
        f"manual sources in your database—not on press release text alone—what should someone "
        f"know about how this type of government announcement generally relates to "
        f"{topic_label_for_button}? Which official sources should they verify for their "
        f"own situation? Official announcement link (for the user to open): {official_url}"
    )
