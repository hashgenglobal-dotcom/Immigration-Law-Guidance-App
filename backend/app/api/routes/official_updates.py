...
from app.services.official_updates_service import (
    ...
    build_ask_prefill_message,
    get_update,
    list_topics,
    list_updates,
)
from app.services.official_updates_topics import primary_topic_for_ask, topic_label
...
async def get_update_detail(
    update_id: int,
    ask_topic: str | None = Query(
        default=None,
        description="User's selected topic filter for Ask button label context.",
    ),
    settings: Settings = Depends(get_settings),
) -> OfficialUpdateDetailResponse:
    try:
    ...
    label = primary_topic_for_ask(row["topic_tags"], ask_topic)
    prefill = build_ask_prefill_message(
        title=row["title"],
        publisher=row["publisher"],
        official_url=row["official_url"],
        published_at=row["published_at"],
        topic_label_for_button=label,
    )
    item = OfficialUpdateDetail(
        **{k: v for k, v in row.items() if k != "raw_excerpt"},
        raw_excerpt=row.get("raw_excerpt"),
        ask_prefill_message=prefill,
    )
